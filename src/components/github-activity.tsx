        ...
        <div className="border border-[hsl(var(--border))] bg-card p-4 sm:p-5 transition-all duration-300 hover:border-[hsl(var(--blueprint-line)/0.5)]">
          <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
            ...
            <span className="font-mono text-[10px] text-muted-foreground tabular-nums">
              {data.contributions.total.toLocaleString()} contributions in the last year
            </span>
          </div>
            ...
            <div className="flex min-w-max">
              ...
              <div className="flex flex-col gap-[3px] mr-2.5">
                {DAY_LABELS.map((label, row) => (
                  <div
                    key={`${label}-${row}`}
                    className="h-[11px] flex items-center font-mono text-[8px] tracking-wider text-muted-foreground/80 uppercase leading-none"
                  >
                    {label}
                  </div>
                ))}
              ...
              <div
                ...
                role="img"
                aria-label={`Contribution graph: ${data.contributions.total.toLocaleString()} contributions in the last year`}
              >
                {data.contributions.weeks.map((week, col) => (
                  <div key={col} className="flex flex-col gap-[3px]">
                    {week.map((cell, row) => (
                      <div
                        ...
                        title={
                          cell.level !== null
                            ? `${cell.date} — ${cell.level} contribution${cell.level === 1 ? '' : 's'}`
                            : undefined
                        }
                        className="size-[11px] border border-[hsl(var(--blueprint-line)/0.15)]"
                        style={{
                          background:
                            cell.level && cell.level > 0
                              ? `hsl(var(--blueprint-line) / ${LEVEL_OPACITY[cell.level]})`
                              : 'hsl(var(--blueprint-line) / 0.05)',
                        }}
                      />
                    ))}
                  </div>
          ...
          <div className="flex items-center justify-end gap-1.5 mt-3">
            <span className="font-mono text-[8px] tracking-wider text-muted-foreground uppercase mr-1">
              Less
            </span>
            {LEVEL_OPACITY.map((opacity, level) => (
              <div
                key={level}
                className="size-[10px] border border-[hsl(var(--blueprint-line)/0.15)]"
                style={{
                  background:
                    opacity > 0 ? `hsl(var(--blueprint-line) / ${opacity})` : 'hsl(var(--blueprint-line) / 0.05)',
                }}
              />
            ))}
            <span className="font-mono text-[8px] tracking-wider text-muted-foreground uppercase ml-1">
              More
            </span>
          </div>
        </div>

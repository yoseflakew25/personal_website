    ...
    <section aria-label="skills" className="my-4 space-y-8 bg-transparent">
      ...
      <ScrollReveal variant="fadeUp" delay={0.2}>
        <div className="space-y-5 w-full">
          <Marquee autoFill pauseOnHover speed={20} gradient gradientColor="#0a0a0f" gradientWidth={80}>
            <SkillsList stacks={FRONTEND_STACKS} />
          </Marquee>

          <Marquee autoFill pauseOnHover direction="right" speed={20} gradient gradientColor="#0a0a0f" gradientWidth={80}>
      ...
      {Object.keys(stacks).map((stack, index) => {
        ...
        return (
          <li
            role="listitem"
            key={index}
            className="mr-2.5 flex w-max items-center gap-2 rounded-sm border px-3 py-2 text-[14px] shadow-sm border-cyber-cyan/15 bg-card/50 text-foreground/80 backdrop-blur-sm hover:border-cyber-cyan/40 hover:shadow-neon-sm hover:text-cyber-cyan transition-all duration-300 font-jetbrains"
          >
